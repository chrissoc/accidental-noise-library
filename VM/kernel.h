...
public:
    ...
    {
        return CInstructionIndex((unsigned int)kernel_.size());
    }
    CInstructionIndex lastIndex()
    {
        return CInstructionIndex((unsigned int)kernel_.size()-1);
    }
...
private:
    ...
    double extractConst(CInstructionIndex index, bool& success);
