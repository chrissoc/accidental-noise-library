    ...
    for(int x=0; x<mw; ++x)
    {
        for(int y=0; y<mh; ++y)
        {
            ...
            if(!wrap)
            {
                ...
                {
                    n[0]=static_cast<float>((map->get(x-1,y)-map->get(x+1,y)) / spacing);
                    n[2]= static_cast<float>((map->get(x,y-1)-map->get(x,y+1)) / spacing);
                }
                normalizeVec3(n);
            }
            else
            {
                ...
                n[0]= static_cast<float>((map->get(x1,y)-map->get(x2,y)) / spacing);
                n[2]= static_cast<float>((map->get(x,y1)-map->get(x,y2)) / spacing);
                normalizeVec3(n);
            }
            if(normalize)
            {
